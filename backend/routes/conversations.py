...
async def _execute_tool_call(tool_name: str, tool_input: dict[str, Any], db: AsyncSession) -> str:
    ...
    try:
        if tool_name == "run_agent":
            ...
            # Force the parsed definition's id to match the DB row id. The
            # definition_json carries its own id field (generated by the
            # frontend editor on create), but checkpoint_store inserts runs
            # with FK workflow_id → workflows.id — so it MUST be the row id,
            # not the embedded definition id, or the INSERT fails with a FK
            # violation. Fix is local to the runner entry points; the JSON
            # stays untouched.
